...
class Valuefunction_TT:
    def __init__(self, valuefunction_prename=None, load_existing_list=False, c_name=None, grad_c_name=None):
        ...
        # print('pol(-1, 0, 1)', self.pol[0]([-1, 0, 1]))
        # input()

        print('self.pol_deg', self.pol_deg)
        self.add_fun = lambda t, x: np.zeros(x.shape[-1])
        self.grad_add_fun = lambda t, x: np.zeros(x.shape[-1])
        self.hess_add_fun = lambda t, x: np.zeros((x.shape[-1], x.shape[-1]))


    def set_add_fun(self, fun, dfun, ddfun=None):
        self.add_fun = fun
    ...
    def solve_linear_HJB(self, data, params):
        ...
        while _n_sweep < n_sweep and rel_val > rel_val_tol and rel_val_test > rel_val_tol or _n_sweep <= 0:
            ...
            # print('val', val)
            old_val, val, self.c_add_fun_list[ind] = self.update_components_np(V, val, mat_list, rew_MC, _n_sweep, P_vec, smin, omega, kminor, adapt, maxranks, add_fun_list, self.c_add_fun_list[ind])
        ...
        print("---------- iterate = " + str(_n_sweep) + " omega = " + str(omega) + " smin = " + str(smin) + " val " + str(val), 'val_test', val_test, 'adapt', adapt) 
        print(V.ranks()) 
        print('val, old_val', val, old_val, 'rel_val', rel_val, '_n_sweep', _n_sweep, 'frob_norm(v)', xe.frob_norm(V), 'validation, oldval', val_test, old_val_test, 'rel_val_test', rel_val_test, 'curr_c =', self.c_add_fun_list[ind])
