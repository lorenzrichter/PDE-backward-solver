...
# Combine the ode and the ansatz function to solve the BSDE in this class
...
class Pol_it:
    def __init__(self, initial_valuefun, ode, polit_params):
        ...
        [self.nos, self.nos_test_set, self.n_sweep, self.rel_val_tol, self.rel_tol, self.max_pol_iter] = polit_params
...
#         vec1 = 1*np.ones((self.ode.n, 1)).squeeze()
...
#         res3 = self.v.add_fun(0.3, vec3)
#         res4 = self.v.hess_add_fun(0.3, vec1)
#         print('hess', res4)
#         print('discr', (res2 - 2*res1 + res3) / eps**2)
#         print('anal', res4[0, 0])
#         input()
    ...
    def solve_HJB(self, start_num = None):
        ...
        for i0 in range(start_num, -1, -1):
            ...
            ind_end = np.minimum(len(self.t_vec_p) - 1, i0+1)
    ...
    def solve_HJB_fixed_time(self):
        ...
        while(rel_diff > self.rel_tol and pol_iter < self.max_pol_iter and rel_diff_eval > self.rel_tol/10):
            ...
            if self.current_time < -1 and self.ode.n == 1:
                plt.figure()
